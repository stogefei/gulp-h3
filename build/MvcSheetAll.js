...
(function ($) {
    ...
    $.MvcSheetUI.Controls.SheetAttachment.Inherit($.MvcSheetUI.IControl, {
        ...
        MobileItemClick: function (fileId, fileUrl, IsImg) {
            ...
            var UserCode = "ID:" + $.MvcSheetUI.SheetInfo.UserCode;
            var UserName = "UserName:" + $.MvcSheetUI.SheetInfo.UserName;
            var date = new Date();
            ...
            var CreateTime = "Time:" + year + '年' + month + '月' + day + '日' + hour + ':' + minute + ':' + second;
            var HJc = (UserCode + UserName + CreateTime );
             var    HJcc = encodeURIComponent(HJc);
            // 预览
            var hideSheet = $.MvcSheetUI.IonicFramework.$ionicActionSheet.show({
                buttons: buttons,
                                 ...
                                 success: function (data) {
                                     ...
                                     if (data && data.success && data.url) {
                                         ...
                                         if (Browse_Extension.indexOf(data.extension) > -1) {
                                             ...
                                             var ios = !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端
                                             if (ios) {
                                                 window.location.href = downloadUrl+"&watermark=" + HJcc;
                                             } else {
                                            	   window.location.href = downloadUrl+"&watermark=" + HJcc;
                                                 //$.MvcSheetUI.IonicFramework.$state.go("form.downLoadFile", {extension: data.extension})
                                             }
        ...
        InitValue: function () {
            if (this.V) {
                ...
                for (var i = 0; i < this.V.length; ++i) {
                	var newname=this.V[i].Name
                	var end = newname .lastIndexOf(".");  
                	var start=newname .lastIndexOf("@");
                	if(start>0){
                		newname=this.V[i].Name .substring(start, end);
                		newname=this.V[i].Name.replace(newname,"")
                	}
                    this.CreateFileElement($.trim(this.V[i].Code), newname, this.V[i].Size, this.V[i].Url, this.V[i].ContentType);
                }
        ...
        CreateFileElement: function (fileid, name, size, url, contentType) {
            ...
            if (url == undefined) {
                ...
                if (!this.IsMobile) {
                    ...
                    //预览文档
                    var actionylStr = $("<a href='#' class='fa fa-file'>&nbsp;预览</a>");
                    actionylStr.unbind("click.fileYlBtn").bind("click.fileYlBtn", this, function (e) {
                        //debugger;
                    	  var UserCode = "ID:" + $.MvcSheetUI.SheetInfo.UserCode;
                          var UserName = "UserName:" + $.MvcSheetUI.SheetInfo.UserName;
                          var date = new Date();
                        ...
                        murl = murl.split(_PORTALROOT_GLOBAL)[0];
                        $.ajax({
                            type: "POST",
                            url: murl + url + "&IsMobile=true&AppLogin=true",
                            success: function (data) {
                             console.log(data)
                                window.open(data.url+"&watermark=" + HJcc);
           
                            }
                        });
                    });
                    actionTd.append(actionylStr);
                    actionTd.append("&nbsp;&nbsp;");
        ...
        MobileInit: function () {
            ...
            if (this.Editable) {
                ...
                $(this.Element).parent().unbind('click.MobileCheckListBox').bind("click.MobileCheckListBox", function () {
                    ionic.$ionicModal.fromTemplateUrl('Mobile/form/templates/checkbox_popover.html?v=201909201706', {
                        scope: ionic.$scope
...
(function ($) {
    ...
    $.MvcSheetUI.Controls.SheetComment.Inherit($.MvcSheetUI.IControl, {
        ...
        RenderMobile: function () {
            ...
            //将审批意见移动到最底部
            // $(this.Element).closest('.list').prependTo($('#mobile-content div.scroll'));
        ...
        // 数据验证
        Validate: function (effective, initValid) {
            // console.log(this.DataField, effective, initValid, '--------------');
            if (!this.Editable)
                return true;
            if (initValid) {
        ...
        _changeSignaturePic: function (panel, picName) {
            ...
            if (picName) {
                panel.html("<img alt='' border='0' src='" + $.MvcSheetUI.PortalRoot + "/TempImages/" + picName + ".jpg' />");
        ...
        AddCommentItem: function (commentObject, commentLine) {
            ...
            if (commentObject.commentType) {
                if (commentObject.commentType === "Sheet__ConsultComment") {
                    commentType = $.Lang("SheetComment.Consult"); // 征询
                    isApprove = false;
                } else if (commentObject.commentType === "Sheet__AssistComment") {
                    commentType = $.Lang("SheetComment.Assist"); // 协办
                    isApprove = false;
                } else if (commentObject.commentType === "Sheet__ForwardComment") {
                    commentType = $.Lang("SheetComment.Forward"); //转办
                    isApprove = false;
                } else if (commentObject.commentType === "Sheet__AdjustComment") { //加签
                    commentType = $.Lang("SheetComment.Adjust");
                    isApprove = false;
                }
            }
            if (isApprove) {
            ...
            if (toUsers.length != 0 && commentType != "") { // 加签  转办 协办 征询
                if (commentObject.isReply) {
                    // commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + toUsers + "</a><a href='javascript:void(0)'>" + userName + "</a> </div>")
                    ...
                    // commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" +'['+ "<a href='javascript:void(0)'>" +'来自'+ toUsers +'的'+commentType + "</a>" +']' +"</a><a href='javascript:void(0)'></a></div>")
                } else if(commentType) {
                    var ActivityText = commentObject.Activity;
                    var type = "[来自" + userName + '的'+ commentType + "]";
                    commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + toUsers+ '['+ ActivityText +']'+ "</a><a href='javascript:void(0)'>" +type +"</a></div>")
                }
                else{
                ...
                if (commentObject.isReply && commentObject.DelegantName) { // 委托
                    userName = "[来自" + userName + '的'+ SheetLanguages.Current.Delegant + "]";
                    commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + userName + "</a></div>");
                } else if(commentObject.Adjust) {  //加签
                    toUsers = "[来自" + toUsers + '的' + SheetLanguages.Current.PlusSign + "]";
                    commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + toUsers + "</a></div>");
                } else if(commentObject.Forward && toUsers.length != 0) {  //转办
                    toUsers = "[来自" + toUsers + '的' + SheetLanguages.Current.Forward + "]";
                    commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + toUsers + "</a></div>");
                } else if(commentObject.Transfer && toUsers.length != 0) {  //转派
                    toUsers = "[来自" + toUsers + '的' + SheetLanguages.Current.Transferred + "]";
                    commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + toUsers + "</a></div>");
                } else { // 正常审批
                    commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + userName + "</a></div>");
                }
            ...
            if (commentObject.Activity && this.ActivityNameVisible && commentType === "") { //正常流程 审批意见
                var ActivityText = commentObject.Activity;
                ActivityText = ActivityText.replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;");
                //修改委托显示位置
                if(commentObject.DelegantName) { // 委托
                    commenby.prepend("<a href='javascript:void(0)'>" +commentObject.DelegantName+ '['+ ActivityText +']'+ "</a>");
                } else if(commentObject.Adjust) {  // 加签
                    commenby.prepend("<a href='javascript:void(0)'>" +userName+ '['+ ActivityText +']'+ "</a>");
                } else if(commentObject.Forward && toUsers.length != 0) {  //转办
                    commenby.prepend("<a href='javascript:void(0)'>" +userName+ '['+ ActivityText +']'+ "</a>");
                } else if(commentObject.Transfer && toUsers.length != 0) {  //转派
                    commenby.prepend("<a href='javascript:void(0)'>" +userName+ '['+ ActivityText +']'+ "</a>");
           ...
           if(commentObject.Text=="NOTEXIT"){
            ...
            if (commentObject.commentType === "Sheet__TransferComment") {
                commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + userName + "</a><span style='font-weight: bold'> 转派给</span><a href='javascript:void(0)'>" + commentObject.transferName + "</a> "+"<span style=\"padding-left:5px;\">" + dateStr + "</span>");
            }
            ...
            if (commentObject.commentType === "Sheet__TransferComment") {
                var noticeText="未沟通客户"
                if(commentObject.circulaters.length>0) {
                    copyTo = "";
                    $(commentObject.circulaters).each(function(i){
                        if(i==0) {
                            copyTo+="<a href='javascript:void(0)' style='padding: 0 5px;'>"+this+"</a>";
                    ...
                    commentBody.append("<div style='margin-bottom:10px'><span style='font-weight: bold;'>抄送:</span>"+copyTo+"</div>")
                }
                if(commentObject.noticeFlag ==1){
                    noticeText = "已沟通客户";
                }
               // commentBody.append("<div>"+noticeText+"</div>")
                var sheetAttach = $("<div><span class='attachment'></span></div>");
                sheetAttach.find(".attachment").SheetAttachment({
                        ...
                        V:commentObject.attachments
                    })
                //commentBody.append()
                commentBody.append(sheetAttach);
            }
        ...
        AddMobileCommentItem: function (commentObject, last) {
            ...
            var commenby=$("<div class='commonFlex'></div>");
            var toUsers = "";
            ...
            if (commentObject.commentType) {
                ...
                else if (commentObject.commentType === "Sheet__AdjustComment") {
                    commentType = $.Lang("SheetComment.Adjust");
                    isApprove = false;
                } else if (commentObject.commentType === "Sheet__TransferComment") {
                    commentType = $.Lang("SheetComment.Transfer");
                    isApprove = false;
                }
            }
            if (isApprove) {
            ...
            // if (toUsers.length != 0 && commentType != "") { // 加签  转办 协办 征询
            //     if (commentObject.isReply) {
            //         commenby.append( $("<div class='comment-by1'><a href='javascript:void(0)'>" + userName + "</a> <span>"+$.Lang("SheetComment.Feedback")+"</span>  <a href='javascript:void(0)'>" + toUsers + "</a> <span>"+$.Lang("SheetComment.Of")+" </span><a href='javascript:void(0)'>[" + commentType + "]</a>]</div>"))
            //     } else {
            //         commenby.append($("<div class='comment-by1'><a href='javascript:void(0)'>" + userName + "</a><span> "+$.Lang("SheetComment.Originate")+"</span>  <a href='javascript:void(0)'>[" + commentType + "]</a><span>, "+$.Lang("SheetComment.Wait")+" </span><a href='javascript:void(0)'>" + toUsers + "</a> <span>"+$.Lang("SheetComment.Feedback")+"</span></div>"));
            //     }
            // }
            // else {
            //     commenby.append($("<div class='comment-by1'><a href='javascript:void(0)'>" + userName + "</a></div>"))
            // }

            if (toUsers.length != 0 && commentType != "") { // 加签  转办 协办 征询
                if (commentObject.isReply) {
                    // commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" + toUsers + "</a><a href='javascript:void(0)'>" + userName + "</a> </div>")
                    commenby = $("<div class='comment-by1'><a href='javascript:void(0)'>" + userName + "</a> "+$.Lang("SheetComment.Feedback")+"  <a href='javascript:void(0)'>" + toUsers + "</a> "+$.Lang("SheetComment.Of")+" [<a href='javascript:void(0)'>" + commentType + "</a>]</div>")
                    // commenby = $("<div class='comment-by'><a href='javascript:void(0)'>" +'['+ "<a href='javascript:void(0)'>" +'来自'+ toUsers +'的'+commentType + "</a>" +']' +"</a><a href='javascript:void(0)'></a></div>")
                } else if(commentType) {
                    var ActivityText = commentObject.Activity;
                    var type = "[来自" + userName + '的'+ commentType + "]";
                    commenby = $("<div class='comment-by1'><a href='javascript:void(0)'>" + toUsers+ '['+ ActivityText +']'+ "</a><a href='javascript:void(0)'>" +type +"</a></div>")
                }
                else{
                    commenby = $("<div class='comment-by1'><a href='javascript:void(0)'>" + userName + "</a> "+$.Lang("SheetComment.Originate")+"  [<a href='javascript:void(0)'>" + commentType + "</a>], "+$.Lang("SheetComment.Wait")+" <a href='javascript:void(0)'>" + toUsers + "</a> "+$.Lang("SheetComment.Feedback")+"</div>")
                }
            }
            else { //正常流程 审批意见
                if (commentObject.isReply && commentObject.DelegantName) { // 委托
                    userName = "[来自" + userName + '的'+ SheetLanguages.Current.Delegant + "]";
                    commenby = $("<div class='comment-by1'><a href='javascript:void(0)'>" + userName + "</a></div>");
                } else if(commentObject.Adjust) {  //加签
                    toUsers = "[来自" + toUsers + '的' + SheetLanguages.Current.PlusSign + "]";
                    commenby = $("<div class='comment-by1'><a href='javascript:void(0)'>" + toUsers + "</a></div>");
                } else  if(commentObject.Forward && toUsers.length != 0) {  //转办
                    toUsers = "[来自" + toUsers + '的' + SheetLanguages.Current.Transferred + "]";
                    commenby = $("<div class='comment-by1'><a href='javascript:void(0)'>" + toUsers + "</a></div>");
                }
            ...
            commenby.find(".comment-by1").append("<span >" + dateStr + "</span>");
          ...
          commentTitle.append(commenby);
            ...
            if (commentObject.commentType === "Sheet__TransferComment") {
                ...
                var sheetAttach = $("<div></div>");
                sheetAttach.SheetAttachment(
                    {
                        AllowBatchDownload: true,
                        FileExtensions: "",
                        L: 24,
        ...
        InitCommentInput: function () {
            ...
            this.CommentInput = $("<textarea class='comment-textarea' onchange=\"this.value=this.value.substring(0,2000)\" style='padding-bottom: 15px' maxlength='2000' placeholder='同意'></textarea>").width(this.Width);
            ...
            if (this.IsMobile) {
                ...
                this.CommentInput = $("<textarea class='comment-textarea CommentInput' rows='4' placeholder='同意'></textarea>");
            } else {
            ...
            $(this.CommentInput).unbind("change.CommentInput").bind("change.CommentInput", this, function (e) {
                ...
                $(that.CommentInput).siblings('.text-length').children('span').html(inputLength)
                $.MvcSheetUI.rejectText = e.target.value;
            });
            ...
            // console.log($.MvcSheetUI.IonicFramework,'$.MvcSheetUI.IonicFramework')
            if (this.IsMobile) {
                ...
                $(this.CommentInput).focus(function () {
                    ...
                    var eleTop = (ionic.$ionicPosition.offset(element).top) / 3;
        ...
        InitFrequentlyUsedCommentsAndSignature: function () {
            ...
            if (this.FrequentCommentVisible) {
                if (!this.IsMobile) {
                    //pc端
                    LatestCommentSel = $("<select  style='max-width:100%' class='select-comment-inner'></select>");
                    LatestCommentSel.css({width: "83%",float: "left"});
                    ...
                    $(LatestCommentSel).unbind("change.LatestCommentSel").bind("change.LatestCommentSel", this, function (e) {
                        if ($(this).val().length > 0) {
                            ...
                            var inputLength = $(this).val().length? $(this).val().length: 0
                            // $(".text-length span").html(inputLength)
                            $(that.CommentInput).siblings('.text-length').children('span').html(inputLength)
                            e.data.Validate();
                        }
                    ...
                    LatestCommentPanel.unbind('click.chooseComments').bind('click.chooseComments', function (e) {
                        ionic.$ionicModal.fromTemplateUrl('Mobile/form/templates/radio_popover2.html?v=201909201706', {
                            scope: ionic.$scope
                        }).then(function (popover) {
    ...
    $.MvcSheetUI.Controls.SheetDropDownList.Inherit($.MvcSheetUI.IControl, {
        ...
        Render: function () {
            ...
            if (!this.Editable) {
                ...
                if (this.Queryable && !this.IsMobile) {
                    $element.select2({placeholder: this.EmptyItemText, minimumResultsForSearch: -1});
                ...
                if ($.MvcSheetUI.CacheData && $.MvcSheetUI.CacheData[cmdParam]) {
                    ...
                    if(Object.prototype.toString.call(cacheData)=="[object Array]"){
                    	for (var i in cacheData) {
                    		var data = cacheData;
                    		//update by xl@Future
                    		if (!$element.find("option[value='" + data[i].key.replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;").replace(/\'/g,"&apos;") + "']") || $element.find("option[value='" + data[i].key.replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;").replace(/\'/g,"&apos;") + "']").length == 0) {
                    			if (that.IsMobile) {
                    				that.AddMobileItem(data[i].key, data[i].value, false);
                    			}
                    			if(!!data[i].value){
                    				data[i].value = (data[i].value).replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;");
                    				$element.append("<option value=\"" + data[i].key.replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;") + "\">" + data[i].value + "</option>");
                    			}
                    		}
                    	}
        ...
        ionicInit: function (ionic) {
            ...
            $(this.Element.parentElement).unbind('click.showChoice').bind('click.showChoice', function (e) {
                ionic.$ionicModal.fromTemplateUrl('Mobile/form/templates/radio_popover.html?v=201909201706', {
                    scope: ionic.$scope
                }).then(function (popover) {
                    popover.scope.header = that.N;
        ...
        _selectItem: function ($element, selectedValue) {
            ...
            if (!this.Editable) {
                ...
                if (textLable.length > 0) {
                	...
                	//var valueText = this.GetText()||selectedValue;
                    ...
                    popover.scope.clickRadio = function (value, text) {
                        for (var i = 0; i < $(that.Element).children("div").length; i++) {
                            ...
                            if ($($($(that.Element).children("div"))[i])[0].innerText === value) {
                                ...
                                $($(that.Element).children("div").children("input")[0]).val(value);
                                // console.log($(that.Element).children("div").children("input")[0],'input');
                            }
                        }
                        that.SetValue(value);
        ...
        Render: function () {
           ...
           this.Element.maxLength="2000";
            ...
            if (this.IsMobile) {
                ...
                $(this.Element).attr("PlaceHolder",'请输入');
                $(this.Element).css("position","relative")
                if (this.Editable) {
                    $(this.Element).parent().append(lengthText)
                }
            }
            ...
            var that = this
            //值改变事件
            $(this.Element).unbind("change.CommentInput").bind("change.CommentInput", this, function (e) {
                var inputLength = e.target.value.length
                $(that.Element).siblings().children('span').html(inputLength)
            });
...
//                contentValue = contentValue.replace(/\"/g,"&quot;");
 //               contentValue = contentValue.replace(/\</g,"&lt;").replace(/\>/g,"&gt;").replace(/\"/g,"&quot;");
                //console.log(contentValue)
                parentContainer.html("<div style=\"overflow-x:auto;display:block;white-space:inherit;padding:7px 0;\" id=\"divRich_" + this.Element.id + "\">" + contentValue + "</div>")
                	.insertAfter($(this.Element));
...
(function ($) {
    ...
    $.MvcSheetUI.Controls.SheetTextBox.Inherit($.MvcSheetUI.IControl, {
        ...
        Render: function () {
            ...
            this.Element.maxLength="200";
            $(this.Element).attr('autocomplete','off');
            var lengthText = $("<p class='text-length textbox-length'><span>0</span>/200</p>");
            // 查看痕迹
            if (this.TrackVisiable && !this.Originate && this.DataField.indexOf(".") == -1) { this.RenderDataTrackLink(); }
            // 移动端设置PlaceHolder
            if (this.IsMobile) {
            ...
            // 绑定焦点事件
            $(this.Element).unbind("focus.SheetTextBox").bind("focus.SheetTextBox", [this], function (e) {
            ...
            if (this.OnKeyUp) {
                $(this.Element).unbind("keyup.SheetTextBox").bind("keyup.SheetTextBox", [this], function (e) {
                    ...
                    $(that.Element).siblings().children('span').html(inputLength)
                    if(inputLength == 200) {
                        $(that.Element).addClass("inputTextError");
                        $(that.Element).siblings().show()
                        setTimeout(function () {
                            $(that.Element).removeClass("inputTextError");
                            $(that.Element).siblings().hide()
                        },3000)
                    }
                });
            }
        ...
        (function ($) {
            ...
            $.MvcSheetUI.Controls.SheetUser.Inherit($.MvcSheetUI.IControl, {
                ...
                ionicInit: function (that, ionic) {
                    ...
                    if (that.DataField == "fetchUserSelect"||that.DataField=="transferUserSelect"||that.DataField=="copyToUserSelect") {
                ...
                HtmlRender: function () {
                    ...
                    if (this.SelectorPanel == null) {
                        ...
                        this.OrgTreePanel = $("<div>").css("max-width", "40%").css("width", "40%").addClass("bordered").css("float", "left").height("100%").css("overflow", "scroll");
                ...
                BindEnvens: function () {
                    ...
                    if (this.IsMobile) {
                        ...
                        $(this.SearchTxtElement).unbind("keyup.SearchTxtElement").bind("keyup.SearchTxtElement", this, function (e) {
                            ...
                            var that = e.data;
                            if(_self.canSearch){
                                _self.canSearch = false;
                                setTimeout(function () {
                                    _self.SearchOrg.apply(that, [that]);
                                    _self.canSearch = true;
                                }, 500)
                            }
                ...
                FocusInput: function () {
                    ...
                    var left = $("body").width() - this.SelectorPanel.width()-this.SelectorPanel.offset().left-100;
                    if(left<0)
                    {
                        this.SelectorPanel.css("left",left+"px")
                    }
                    ...
                    if (this.IsMobile) {
                        ...
                        var item = $("<div>").addClass("task").append("<i style='float:left;line-height: 18px;' class='task-sort-icon fa  " + node.Icon + "'></i>")
                            .append("<span title='" + titleText+ " ' class='task-title' " +
                                "style='word-break: break-all;display: inline-block;width: 80%;margin-top: 0;overflow: hidden;text-overflow: ellipsis;padding-left: 4px'>" + displayText + "</span>");
